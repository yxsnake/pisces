...
public class ConfigBuilder {
    ...
    private List<TableInfo> processTable(List<TableInfo> tableList, StrategyConfig config) {
        for (TableInfo tableInfo : tableList) {
            ...
            // http request client
            if (StringUtils.isNotBlank(globalConfig.getHttpClientName())) {
                tableInfo.setHttpClientName(String.format(globalConfig.getHttpClientName(), entityName));
            } else {
                tableInfo.setHttpClientName(entityName + ConstVal.HTTP_CLIENT);
            }
            // meta object handler
            if (StringUtils.isNotBlank(globalConfig.getMetaObjectHandlerName())) {
                tableInfo.setMetaObjectHandler(String.format(globalConfig.getMetaObjectHandlerName(), entityName));
            } else {
                tableInfo.setMetaObjectHandler(entityName + ConstVal.META_OBJECT_HANDLER);
            }
            // 检测导入包
